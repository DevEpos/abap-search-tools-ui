...
public class CdsAnalysisManager {
	 ...
	 * @return     the found CDS analysis or <code>null</code> if none could be
	 *             found
	 */
	public CdsAnalysis getExistingAnalysis(final CdsAnalysisKey key) {
		synchronized (this) {
			return this.analysesMap.get(key);
		}
	}
	...
	public void registerAnalysis(final CdsAnalysisKey analysisKey, final CdsAnalysis newAnalysis) {
		this.analysesMap.put(analysisKey, newAnalysis);
	}
	...
	public void removeAnalysis(final CdsAnalysis analysis) {
		synchronized (this) {
			...
			this.analysesMap.values().remove(analysis);
		}
	...
	public void removeAll() {
		synchronized (this) {
			final List<CdsAnalysis> old = this.analyses;
			this.analyses = new LinkedList<>();
			this.analysesMap = new HashMap<>();
	...
	public CdsAnalysis[] getAnalyses() {
		synchronized (this) {
			return this.analyses.toArray(new CdsAnalysis[this.analyses.size()]);
		}
	}
	...
	private void establishHistoryLimit() {
		...
		for (final CdsAnalysis analysis : analyses) {
			if (!viewManager.isAnalysisShown(analysis)) {
