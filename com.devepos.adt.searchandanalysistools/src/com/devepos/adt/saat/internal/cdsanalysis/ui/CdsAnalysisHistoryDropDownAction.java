...
class CdsAnalysisHistoryDropDownAction extends Action implements IMenuCreator {
	...
	public Menu getMenu(final Control parent) {
		...
		final CdsAnalysis currentAnalysis = this.analysisView.getCurrentAnalysis();
		...
		addActionToMenu(this.menu, new Action(Messages.CdsAnalysis_ManageHistory_xmit) {
			@Override
			public void run() {
				openManagePageDialog();
			}
		});
		addActionToMenu(this.menu, new Action(Messages.CdsAnalysis_ClearHistoryAction_xmit) {
			@Override
			public void run() {
	...
	private class ShowAnalysisAction extends Action {
		...
		public void runWithEvent(final Event event) {
			if (isChecked()) {
				CdsAnalysisManager.getInstance().showAnalysis(this.analysis);
			}
		}
	}
}