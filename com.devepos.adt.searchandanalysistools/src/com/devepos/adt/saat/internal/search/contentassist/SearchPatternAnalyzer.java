...
public class SearchPatternAnalyzer {
	...
	public void checkParameters(final String searchPattern) throws CoreException {
		...
		for (final String part : condensedPattern.split(SPACE)) {
			if (!part.isEmpty() && !isParameter(part) && !part.equalsIgnoreCase(searchTerm)) {
				final String errorMessage = NLS.bind(Messages.SearchPatternAnalyzer_ErrorInvalidSearchParameter_xmsg, part);
				throw new CoreException(new Status(IStatus.ERROR, SearchAndAnalysisPlugin.PLUGIN_ID, errorMessage));
			}
		}
	}
	...
	public List<IContentProposal> getFilters(final String query) {
		...
		for (final ISearchParameter searchParam : this.parameters) {
			// no query part so every parameter gets added to the proposal list
			if (lastPart.isEmpty()) {
				filters.add(new SearchFilterProposal(searchParam.getLabel(), searchParam.getImage(), searchParam.getDescription(),
				...
				if (searchParam.getLabel().startsWith(lastPart)) {
					filters.add(new SearchFilterProposal(searchParam.getLabel(), searchParam.getImage(),
						searchParam.getDescription(), lastPart, searchParam instanceof ISearchProposalProvider));
				}
	...
	private boolean checkParameterValuesProvided(final String part, final ISearchParameter param) throws CoreException {
		...
		if (part.length() <= (param.getLabel() + PARAM_KEY_END).length()) {
			throw new CoreException(new Status(IStatus.ERROR, SearchAndAnalysisPlugin.PLUGIN_ID,
				NLS.bind(Messages.SearchPatternAnalyzer_ErrorIncompleteSearchParameter_xmsg, part)));
		}
	...
	private boolean isParameter(final String part) throws CoreException {
		if (this.parameters != null) {
			for (final ISearchParameter param : this.parameters) {
				if (part.startsWith(param.getLabel() + PARAM_KEY_END) && checkParameterValuesProvided(part, param)) {
					return true;
				}
			}
		}
	...
	private boolean isParameterProposal(final String query) {
		final String lastPart = getStringToAnalyse(query);
		for (final ISearchParameter parameter : this.parameters) {
	...
	private boolean isValueInProposalList(final List<IContentProposal> proposalList, final String value) {
	...
	private void updateSearchParameters() {
		...
		this.parameters = this.parameterHandler.getParameters();
	}
