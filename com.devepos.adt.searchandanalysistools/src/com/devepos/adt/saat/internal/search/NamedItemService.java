...
public class NamedItemService implements INamedItemService {
	...
	@Override
	public INamedItem[] getNamedItems(final NamedItemType type, final int maxResults, final String name, final String description,
		final String data) {
		...
		if (template != null) {
			...
			final IRestResource resource = AdtRestResourceFactory.createRestResourceFactory()
				.createRestResource(resourceUri,
					AdtSystemSessionFactory.createSystemSessionFactory().createStatelessSession(this.destination));
	 ...
	 * @param data
	 */
	private void fillTemplate(final IAdtUriTemplate template, final int maxResults, final String name, final String description,
		final String data) {
		if (maxResults > 0) {
	...
	private IAdtUriTemplate getNamedItemTemplate(final NamedItemType type) {
		return new ObjectSearchUriDiscovery(this.destination).getTemplateByDiscoveryTerm(type.getDiscoveryTerm());
	}
