...
public class NamedItemService implements INamedItemService {
	...
	public INamedItem[] getNamedItems(final NamedItemType type, final int maxResults, final String name,
		...
		if (template != null) {
			...
			final IRestResource resource = AdtRestResourceFactory
					.createRestResourceFactory()
						.createRestResource(resourceUri,
								AdtSystemSessionFactory
										.createSystemSessionFactory()
											.createStatelessSession(this.destination));
	...
	private IAdtUriTemplate getNamedItemTemplate(final NamedItemType type) {
		switch (type) {
		case ANNOTATION:
			return new ObjectSearchUriDiscovery(this.destination).getAnnotationValueHelpTemplate();
		case ANNOTATION_VALUE:
			return new ObjectSearchUriDiscovery(this.destination).getAnnotationValueValueHelpTemplate();
		case CDS_FIELD:
			return new ObjectSearchUriDiscovery(this.destination).getCdsFieldValueHelpTemplate();
		case TABLE_FIELD:
			return new ObjectSearchUriDiscovery(this.destination).getTableFieldValueHelpTemplate();
		case DB_ENTITY:
			return new ObjectSearchUriDiscovery(this.destination).getDbEntityValueHelpTemplate();
		case CDS_TYPE:
			return new ObjectSearchUriDiscovery(this.destination).getCdsTypeValueHelpTemplate();
		case API_STATE:
			return new ObjectSearchUriDiscovery(this.destination).getReleaseStateValueHelpTemplate();
		case CDS_EXTENSION:
			return new ObjectSearchUriDiscovery(this.destination).getCdsExtensionValueHelpTemplate();
		default:
			return null;
		}
	}
