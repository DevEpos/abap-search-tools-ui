 ...
 * @author stockbal
 */
public class ElementInfoCollectionContentHandler implements IContentHandler<IElementInfoCollection> {
	...
	@Override
	public IElementInfoCollection deserialize(final IMessageBody body, final Class<? extends IElementInfoCollection> dataType) {
	...
	private void deserializeElementInfoList(final IXmlElement xmlElem, final List<IElementInfo> elemInfoList) {
		for (final IXmlElement elementInfoElement : xmlElem.getChildren()) {
			deserializeElementInfo(elementInfoElement, elemInfoList);
		}
	}

	private void deserializeElementInfo(final IXmlElement xmlElemList, final List<IElementInfo> elemInfoList) {
		...
		if (name != null && !name.isEmpty() && uri != null && !uri.isEmpty() && type != null && !type.isEmpty()) {
			...
			adtObjRefElemInfo.setElementInfoProvider(new ObjectSearchElementInfoProvider(this.destinationId, adtObjectRef));
			elementInfo = adtObjRefElemInfo;
		} else if (xmlElemList.hasChild(IXmlTags.EL_ELEMENT_INFO)) {
			final IElementInfoCollection collection = new ElementInfoCollection(name, rawName, null, description);
	...
	private void addAdditionalResultInformation(final Map<String, String> properties, final IElementInfo elementInfo) {
