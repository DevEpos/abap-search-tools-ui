...
public class AnnotationSearchParameter extends NamedItemProposalProvider
	...
	public AnnotationSearchParameter(final IAbapProjectProvider projectProvider) {
		...
		this.parameterName = QueryParameterName.ANNOTATION;
		this.image = SearchAndAnalysisPlugin.getDefault().getImage(IImages.ANNOTATION_PARAM);
		this.annotationValueProposalProvider = new NamedItemProposalProvider(projectProvider, QueryParameterName.ANNOTATION,
	...
	public List<IContentProposal> getProposalList(final String query) throws CoreException {
		...
		if (query != null && query.contains(KEY_VAL_SEPARATOR)) {
			...
			return this.annotationValueProposalProvider.getProposals(annotationValueQuery, annotationValueQuery,
		...
		if (annoFilterValue.contains(KEY_VAL_SEPARATOR)) {
			...
			if (StringUtil.startsWithNegationCharacter(annoKey)) {
				throw new CoreException(new Status(IStatus.ERROR, SearchAndAnalysisPlugin.PLUGIN_ID,
					NLS.bind(Messages.SearchPatternAnalyzer_NoValuePartPossibleIfKeyIsNegated_xmsg, getLabel())));
			}
