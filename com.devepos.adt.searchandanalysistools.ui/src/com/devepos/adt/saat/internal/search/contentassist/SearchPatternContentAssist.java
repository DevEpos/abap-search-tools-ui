...
public class SearchPatternContentAssist extends ContentProposalProvider {
	...
	public SearchPatternContentAssist(final Text control, final SearchPatternAnalyzer patternAnalyzer) {
		super(control);

		this.text = control;
		this.queryResults = new ArrayList<>();
		this.contentProvider = new SearchPatternContentProvider(patternAnalyzer);
		...
		this.contentChangeListener = resultList -> {
			showProposals(resultList);
		};

		this.proposalListener = new IContentProposalListener() {

			@Override
			public void proposalAccepted(final IContentProposal proposal) {
				if (proposal instanceof SearchFilterProposal && ((SearchFilterProposal) proposal).hasProposalSupport()) {
			...
			private void adjustMatch(final SearchParameterProposal proposal) {
				...
				if (proposal.getKey().equalsIgnoreCase(insertedText)) {
					final String queryAfterInsert = SearchPatternContentAssist.this.text.getText();
					final int caretPosition = SearchPatternContentAssist.this.text.getCaretPosition();
					final int caretBeforeLexem = caretPosition - insertedText.length() - proposal.getLexeme().length();
					final String adjustedProposal = queryAfterInsert.substring(0, caretBeforeLexem) + insertedText;
					final String query = adjustedProposal + queryAfterInsert.substring(caretPosition, queryAfterInsert.length());
					SearchPatternContentAssist.this.text.setText(query);
					SearchPatternContentAssist.this.text.setSelection(adjustedProposal.length());
				}
			}
		};

		getContentProposalAdapter().addContentProposalListener(this.proposalListener);
		this.contentProvider.addContentChangeListener(this.contentChangeListener);
	...
	public void dispose() {
		...
		if (this.contentChangeListener != null && this.contentProvider != null) {
			this.contentProvider.removeContentChangeListener(this.contentChangeListener);
		}

		if (this.proposalListener != null) {
			adapter.removeContentProposalListener(this.proposalListener);
		}
	}
	...
	public IContentProposal[] getProposals(final String contents, final int position) {
		...
		result.addAll(this.contentProvider.computeProposals(query));
	...
	private int calculatePopupWidth(final PopupDialog popup) {
		int width = (int) Math.max(300L, Math.round(this.text.getSize().x * 0.5));
	...
	private void showProposals(final List<IContentProposal> newProposals) {
		this.queryResults.clear();
		this.queryResults.addAll(newProposals);
