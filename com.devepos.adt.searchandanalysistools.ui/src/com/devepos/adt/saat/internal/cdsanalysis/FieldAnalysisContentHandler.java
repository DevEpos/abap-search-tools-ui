...
public class FieldAnalysisContentHandler extends AdtObjectElementInfoContentHandlerBase {
	...
	public FieldAnalysisContentHandler(final String destinationId, final boolean topDown, final boolean searchCalcFields,
	...
	private void deserializeNodeInfo(final IXmlElement xmlElement, IElementInfo parent, final boolean isRoot) {
	...
	private IElementInfo extractElementInfo(final IXmlElement xmlElement, final IElementInfo parent, final boolean isRoot) {
		...
		if (xmlElement.hasChild(IXmlTags.EL_ELEMENT_INFO)) {
			final IAdtObjectReferenceElementInfo adtObjInfo = createAdtObjectRefInfo(name, displayName, type, uri, fieldName,
				false, properties);
			elementInfo = adtObjInfo;
		} else {
			if (this.topDown) {
				final IAdtObjectReferenceElementInfo adtObjRefInfo = createAdtObjectRefInfo(name, displayName, type, uri,
				...
				final IAdtObjectReferenceElementInfo adtObjRefInfo = createAdtObjectRefInfo(name, displayName, type, uri,
					fieldName, true, properties);
				adtObjRefInfo.setElementInfoProvider(new FieldWhereUsedInCdsElementInfoProvider(this.destinationId, displayName,
					fieldName, this.searchCalcFields, this.searchDbViews));
				elementInfo = adtObjRefInfo;
			}
		}
