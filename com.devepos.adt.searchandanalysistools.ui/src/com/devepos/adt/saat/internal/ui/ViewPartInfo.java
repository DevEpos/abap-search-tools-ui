...
public abstract class ViewPartInfo extends ScrolledComposite {
	...
	protected void createInformationText() {
		...
		this.content.setBackground(JFaceColors.getBannerBackground(Display.getDefault()));
		...
		addControlListener(new ControlAdapter() {
			...
			public void controlResized(final ControlEvent e) {
				...
				ViewPartInfo.this.setMinSize(ViewPartInfo.this.content.computeSize(r.width, -1));
			}
		});
	}
	...
	protected Label addLabel(final String text) {
		final Label label = new Label(this.content, SWT.LEFT | SWT.TOP | SWT.WRAP);
		label.setText(text);
	...
	protected StyledText addStyledText(final String text) {
		final StyledText styledText = new StyledText(this.content, SWT.WRAP);
	...
	private void addTitle() {
		...
		setWrappingLayoutData(new Composite[] { title });
	}
	...
	private void addHelpLink() {
		...
		final Link link = new Link(this.content, SWT.NONE);
		link.setBackground(JFaceColors.getBannerBackground(Display.getDefault()));
		link.setText(String.format("<a href=\"%s\">%s %s</a>", ViewPartInfo.HELP_LINK, "Learn more about", helpLinkText));
